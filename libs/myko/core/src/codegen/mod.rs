...
mod tests {
    ...
    /// Both tests in this module call generate_item_types("bindings")
    /// against the same real, shared, non-isolated filesystem directory —
    /// serialize them so one test's wipe/regenerate doesn't race the
    /// other's, mirroring the scheduler_test_serial pattern used elsewhere
    /// in this codebase for shared-state test interference.
    fn bindings_dir_test_serial() -> std::sync::MutexGuard<'static, ()> {
    ...
    #[test]
    fn generate_index() {
        let _serial = bindings_dir_test_serial();
    ...
    fn generate_item_types_removes_stale_files_from_a_previous_run() {
        ...
        let stale_path = Path::new(dir).join("StaleTestArtifact.ts");
        fs::write(
            &stale_path,
        ...
        generate_item_types(dir).expect("Failed to generate index.ts");
        ...
        let index_contents =
            fs::read_to_string(Path::new(dir).join("index.ts")).expect("index.ts must exist");
        assert!(
