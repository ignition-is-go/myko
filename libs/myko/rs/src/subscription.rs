...
pub struct Subscription<T: Eventable, PT: Clone> {
    pub state: HashMap<String, T>,
    pub tx: tokio::sync::mpsc::Sender<Vec<T>>,
    pub query: Box<PT>,
}

pub trait Publisher<T: Eventable, PT: Clone> {
    fn publish(&self) -> Result<(), TrySendError<Vec<T>>>;
    fn handle(&mut self, item: &T, event_type: MEventType);
}

// impl<T: Eventable + PartialEq, PT: Clone> Publisher<T, PT> for Subscription<T, PT> {
//     fn handle(&mut self, item: &T, event_type: MEventType) {
