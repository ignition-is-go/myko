        ...
        while let Some(msg) = receiver.recv().await {
            match msg {
                PeerMessage::Discovered(server) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        let connected = if already_connected {
                            ...
                            let result = tokio::time::timeout(Duration::from_secs(5), conn_rx)
                                .await
                                .unwrap_or(Ok(false))
                                .unwrap_or(false);
                            drop(conn_guard);
                        ...
                        let (verify_tx, verify_rx) = tokio::sync::oneshot::channel::<bool>();
                        ...
                        let verified = tokio::time::timeout(Duration::from_secs(3), verify_rx)
                            .await
                            .unwrap_or(Ok(false))
                            .unwrap_or_else(|_| {
                ...
                PeerMessage::Connected { server_id, client } => {
                    ...
                    let client_for_close = keepalive_client.clone();
                    let status_guard =
                        keepalive_client
                            .connection_status()
                                ...
                                if let hypha::Signal::Value(status) = signal {
                                    match &**status {
                                        ...
                                        ConnectionStatus::Disconnected => {
                                            ...
                                            if let Err(e) = keepalive_sender
                                                .try_send(PeerMessage::Disconnected { server_id })
                                            {
                    ...
                    keepalive_client.connection_status().own(status_guard);
